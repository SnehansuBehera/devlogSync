...
export const createTask = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!title || !assigneeUser || !startDate || !projectId) {
      res.status(400).json({ error: "Invalid task or assignee data" });
      return;
    }
    ...
    const createdTask = await prisma.task.create({
      data: {
        ...
        startDate: startDate ? new Date(Number(startDate)) : new Date(),
        dueDate: dueDate ? new Date(Number(dueDate)) : new Date(),
        status: "PENDING",
