...
export const emailDailyReport = async (userdetails: any, ownerEmail: string, exportUrl: string, date: string) => { 
  try {
    await transporter.sendMail({
      from: `${userdetails.firstName} ${userdetails.lastName}" <${process.env.EMAIL_USER}>`,
      to: ownerEmail,
      replyTo: userdetails.email,
      subject: `${userdetails.username} Daily Report [ ${date || new Date().toISOString().slice(0, 10)} ]`, 
      html: `
